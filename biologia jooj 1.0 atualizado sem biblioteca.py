...
cores_aleatorias = [cores['preto'], cores['vermelho'], cores['verde'], cores['amarelo'], cores['azul'], cores['magenta'], cores['ciano']]
...
def alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais):
    ...
    if decisao == alternativa_certa:
        ...
        print_bonito(cores['verde']+"Correto\n"+cores['reset'])
    else:
        print_bonito(cores['vermelho']+"Errada\n"+cores['reset'])
        if rolar_dado == 0:
            pontos_totais /= 2
            jogador['pontuacao'] += pontos_totais
...
while jogo_ativado == 1:
    os.system('cls')
    print(cores['verde']+biologia_titulo,"\n")
    input("               Pressione Enter")
    while True:
        jogadores_participando.clear()
        os.system('cls')
        print_bonito(cores['reset']+"Quantos jogadores vai jogar (max. 7)?\n")
        ...
        if qnt_de_jogadores.isdigit() and 1 <= int(qnt_de_jogadores) <= 7:
            ...
            for jogador in jogadores_participando:
                print(jogador['cor']+f"Jogador: {jogador['nome']} |Sua rodada: {jogador['entrada']}" + cores['reset'])
            print_bonito("\nOs dados estão corretos?\n")
            print_bonito(cores["amarelo"]+("(1. Sim) (2. Não)\n"))
            decisao = input(cores['reset'])
            if decisao == "1":
                break
    ...
    while gameplay == 1:
        for jogador in jogadores_participando:
            ...
            if selecao_de_dificuldade == 1:
                print(f"{cores['verde'] +facil_texto+ cores['reset']}\n")
                pontos_totais = 2
            elif  selecao_de_dificuldade == 2 or selecao_de_dificuldade == 4:
                print(f"{cores['amarelo'] +medio_texto+ cores['reset']}\n")
                selecao_de_dificuldade = 2
                pontos_totais = 4
            elif selecao_de_dificuldade == 3:
                print(f"{cores['vermelho'] +dificil_texto+ cores['reset']}\n")
                pontos_totais = 6

            print_bonito(f"Você, jogador {jogador['cor']+jogador['nome'] + cores['reset']}, gostaria de rolar um dado para multiplicar seu ganho de pontos?\n")
            print_bonito(cores["amarelo"]+"(1. Sim) (2. Não)\n")
            decisao = input(cores['reset'])
            os.system('cls')
            if decisao == "1":
                ...
                print_bonito(f"O jogador {jogador['cor']+jogador['nome'] + cores['reset']} Rolou um {rolar_dado}\n")
            ...
            if selecao_de_dificuldade == 1:
                if selecao_de_perguntas == 1:
                    print_bonito(cores['reset']+"O que são briofitas?\n")
                    print_bonito(cores["amarelo"]+"(1. Plantas não vasculares, como musgos e hepáticas.) \n(2. Plantas vasculares que se reproduzem por esporos, como samambaias.)  \n(3. Plantas vasculares que produzem sementes expostas, como pinheiros.)\n")
                    alternativa_certa = "1"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                    
                elif selecao_de_perguntas == 2:
                    print_bonito(cores['reset']+"Quais são os principais exemplos de briofitas?\n")
                    print_bonito(cores["amarelo"]+"(1. Samambaias, avencas e licófitas.) \n(2.  Musgos, hepáticas e antóceros.)  \n(3. Coníferas, cicadófitas e ginkgo.)\n")
                    alternativa_certa = "2"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 3:
                    print_bonito(cores['reset']+"Como as gimnospermas se reproduzem?\n")
                    print_bonito(cores["amarelo"]+"(1. Através de sementes formadas em cones.) \n(2. Através da polinização e formação de frutos.)  \n(3. Por meio de esporos produzidos em esporângios.)\n")
                    alternativa_certa = "1"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 4:
                    print_bonito(cores['reset']+"As briofitas possuem raízes verdadeiras?\n")
                    print_bonito(cores["amarelo"]+"(1. Sim, possuem raízes verdadeiras que ajudam na absorção de água e nutrientes.) \n(2. Não, pois ela não precisa de nutrientes)  \n(3. Não, elas têm estruturas chamadas rizóides que não absorvem água como raízes verdadeiras.)\n")
                    alternativa_certa = "3"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 5:
                    print_bonito(cores['reset']+"Qual é o papel das flores nas angiospermas?\n")
                    print_bonito(cores["amarelo"]+"(1. Para armazenar água da chuva e fazer fotossíntese) \n(2. Para criar frutos)  \n(3. Atraem polinizadores e facilitam a reprodução.)\n")
                    alternativa_certa = "3"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 6:
                    print_bonito(cores['reset']+"O que são angiospermas?\n")
                    print_bonito(cores["amarelo"]+"(1. Organismos unicelulares que habitam ambientes aquáticos.) \n(2. Vegetais que não possuem flores e frutos.)  \n(3. Plantas vasculares que produzem sementes nuas.)\n")
                    alternativa_certa = "3"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 7:
                    print_bonito(cores['reset']+"Como as angiospermas se reproduzem?\n")
                    print_bonito(cores["amarelo"]+"(1. Exclusivamente por reprodução assexuada.) \n(2. Através da polinização e formação de frutos.)  \n(3. Por meio de esporos, como os fetos.)\n")
                    alternativa_certa = "2"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 8:
                    print_bonito(cores['reset']+"O que é a polinização e por que é importante para as angiospermas?\n")
                    print_bonito(cores["amarelo"]+"(1. Transferência de pólen de uma flor para outra, essencial para a reprodução.) \n(2. É a produção de hormônios vegetais que regulam o crescimento.)  \n(3. É o processo de formação do solo a partir da decomposição de matéria orgânica.)\n")
                    alternativa_certa = "1"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 9:
                    print_bonito(cores['reset']+"Quais são os principais exemplos de pteridófitas?\n")
                    print_bonito(cores["amarelo"]+"(1. Coníferas, cicadófitas e ginkgo.) \n(2. Samambaias, avencas e licófitas.)  \n(3. Musgos, hepáticas e antóceros.)\n")
                    alternativa_certa = "2"
                    alternativa_pergunta_slafodase(alternativa_certa, jogador, pontos_totais)
                elif selecao_de_perguntas == 10:
                    print_bonito(cores['reset']+"As pteridófitas têm raízes verdadeiras?\n")
        ...
        for jogador in jogadores_participando:
            ...
            if rodadas == 1:
                ...
                print_bonito(f"Parabéns {jogador_vencedor['cor']+jogador_vencedor['nome'] + cores['reset']}! \nVocê venceu o jogo com {jogador_vencedor['pontuacao']} pontos!\n\n")
